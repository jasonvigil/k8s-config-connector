...
var _ refsv1beta1.RefNormalizer = &EnvgroupAttachmentRef{}
...
// Normalize ensures the "External" field is specified for a given EnvgroupAttachmentRef,
...
func (r *EnvgroupAttachmentRef) Normalize(ctx context.Context, reader client.Reader, defaultNamespace string) error {
	if r.External == "" {
		...
		u.SetGroupVersionKind(ApigeeEnvgroupAttachmentGVK)
		if err := reader.Get(ctx, key, u); err != nil {
			if apierrors.IsNotFound(err) {
				return k8s.NewReferenceNotFoundError(u.GroupVersionKind(), key)
			}
			return fmt.Errorf("reading referenced %s %s: %w", ApigeeEnvgroupAttachmentGVK, key, err)
		}
	...
	if _, err := NewEnvgroupAttachmentIdentity(r.External); err != nil {
		return err
	}
	return nil
}
